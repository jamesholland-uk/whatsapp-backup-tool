...
async function exportGroups(waClient, chats, contactMap, selfUser) {
  ...
  console.log(
    'Phase 1 complete. The rejoin playbook is in export/groups.md — keep that even if Phase 2 fails.',
  );
  return records;
}
...
async function run() {
  ...
  try {
    ...
    console.error(
      `\nPhase 2 failed. Phase 1 results in export/groups.md and export/groups.csv are still valid.`,
    );
    throw err;
  }
  ...
  console.log('Review export/groups.md, export/groups.csv and export/chat_summary.csv before deleting the account.');
  console.log('=====================================\n');
