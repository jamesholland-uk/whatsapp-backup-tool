...
A teenager's WhatsApp account is being deleted and replaced with a new parent-managed account on the same phone number. Deleting the account **removes him from every group**, and the new account starts empty — same number or not. Groups only come back if an existing admin re-adds him, or he uses an invite link.

So the point of this tool is **not** a perfect message archive. It is to capture, before deletion, the information needed to **get back into his groups afterwards**: group names, who the admins are, their phone numbers, and any invite links. Chat text is a secondary nice-to-have.
...
- **Reliable:** group list, admin names and numbers, invite links, "is he an admin?"
...
**This is the highest-stakes logic in the tool.** If the script cannot recognise the account owner inside a group's participant list, it reports him as "not an admin" everywhere, the sole-admin warning prints "none", and he deletes the account leaving groups orphaned with no admin. That is unrecoverable. Treat this as the one thing that must be provably right.
...
**This phase must not be skipped, and it is not resumable-by-append.** Each run **rewrites** [`groups.csv`](export/groups.csv) and [`groups.md`](export/groups.md) from scratch: truncate, then write the current full snapshot. Phase 1 is cheap, and appending across a crash would duplicate rows and make the group count untrustworthy — and that count is what the family checks against memory before doing something irreversible. Flush after each group so a mid-pass crash still leaves a readable partial file, but the *next* run starts the roster clean and picks up newly warmed groups. **`progress.json` is not used in Phase 1.**
...
The README is the runbook a non-technical parent follows, not a stub. It must contain:
...
- **After the export, in this order:**
  1. Review `groups.md`, `groups.csv` and `chat_summary.csv`. Open any ordinary group still flagged participants-not-loaded in WhatsApp Web, then rerun the script.
